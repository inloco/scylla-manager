...
// snapshot tag. This allows to fix #3873 without reformating snapshot tag.
// Note that it does not fix situations where two SM instances are running
// backup to the same bucket.
func NewSnapshotTag() string {
	_snapshotTagGuard.mu.Lock()
	defer _snapshotTagGuard.mu.Unlock()

	tag := backupspec.NewSnapshotTag()
	if tag == _snapshotTagGuard.lastTag {
		// Sleep is safer than generating the next available snapshot tag because some
		// parts of the code might assume that snapshot tag cannot be from the future.
		time.Sleep(time.Second)
		tag = backupspec.NewSnapshotTag()
	}
	_snapshotTagGuard.lastTag = tag
	return tag
}

// _snapshotTagGuard is a global storage for the last generated snapshot tag.
// It should be accessed only via NewSnapshotTag function.
var _snapshotTagGuard struct {
	mu      sync.Mutex
	lastTag string
}
